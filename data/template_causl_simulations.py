...
def calculate_ecdf(Z):
    ...
    return jnp.vstack(ecdf_transforms)
...
def frugal_fitting(X, Y, use_marginal_flow=False, Z_disc=None, Z_cont=None, seed=0, frugal_flow_hyperparams={}):
    ...
    if Z_disc is None and Z_cont is None:
        ...
        uz_samples = jnp.vstack([uz_disc_samples, uz_cont_samples])
    # Learn Frugal Flow
...
def run_simulations(data_generating_function: callable, seed: int, num_samples: int, num_iter: int, causal_params: list, hyperparams_dict: dict) -> pd.DataFrame:
    ...
    for i in range(num_iter):
        Z, X, Y = data_generating_function(num_samples, seed=seed+i, causal_params=causal_params).values()
        frugal_fit = causl_py.frugal_fitting(
            X, Y, Z_cont=Z, seed=seed+i, frugal_flow_hyperparams=hyperparams_dict
        )
...
def generate_gaussian_samples(N, causal_params, seed=0):
    ...
    pars <- list(Z1 = list(beta = 0, phi=1),
                 Z2 = list(beta = c(1,1), phi=1),
                 Z3 = list(beta = c(1,1), phi=1),
                 Z4 = list(beta = c(0,1,1,1), phi=0.5),
    ...
    data_samples <- causalSamp({N}, formulas=list(list(Z1~1, Z2~Z1, Z3~Z1, Z4~Z3+Z2+Z1), X~Z1+Z2+Z3, Y~X, ~1), family=fams, pars=pars)
    """
...
def generate_discrete_samples(N, causal_params, seed=0):
    disc_rscript = f"""
    library(causl)
    forms <- list(list(Z1 ~ 1, Z2 ~ 1, Z3 ~ 1, Z4 ~ 1), X ~ Z1*Z2+Z3+Z4, Y ~ X, ~ 1)
    fams <- list(c(1,5,5,1), 5, 1, 1)
    pars <- list(Z1 = list(beta=0, phi=1),
                Z2 = list(beta=0),
                Z3 = list(beta=0),
                Z4 = list(beta=0, phi=2),
