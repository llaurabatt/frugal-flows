...
class MaskedIndependent(AbstractBijection):
    """Independent per-coordinate bijection.

    Each coordinate is transformed by its own elementwise ``transformer``
    (e.g. a rational-quadratic spline). The transformer parameters are produced
    by an MLP whose first-layer weights are zero-masked, which makes them
    constant in the input — i.e. each dimension has its own trainable,
    input-independent transformer. This is the correct unconditional
    parameterisation for fitting marginal CDFs independently across dimensions
    (see ``basic_flows.masked_independent_flow``).
